...
class LFVHEMuAnalyzerMVABDTfitBDTcutBDT(MegaBase):
    tree = 'em/final/Ntuple'
    def __init__(self, tree, outfile, **kwargs):

        self.channel='EMu'
        super(LFVHEMuAnalyzerMVABDTfitBDTcutBDT, self).__init__(tree, outfile, **kwargs)
...
#                print "btagweight,nbtagged,row.jb1pt,row.jb1hadronflavor,row.jb2pt,row.jb2hadronflavor"," ",btagweight," ",nbtagged," ",row.jb1pt," ",row.jb1hadronflavor," ",row.jb2pt," ",row.jb2hadronflavor
            ...
            for sys in self.sysdir:
                ...
                MVAval=MVAcutvalue
                if jn==0 or jn==1:
                    if MVAcutvalue<-0.15:continue
                else:
                    if MVAcutvalue<-0.2:continue
