...
mc_samples = [
    ...
    'VBF_LFV_HToMuTau_M-125_13TeV-powheg-pythia8',
...
for x in mc_samples:
        files.extend(glob.glob('results/%s/LFVHEMuAnalyzerMVA/%s.root' % (jobid, x)))
...
outputdir = 'plots/%s/LFVHEMuAnalyzerMVA/%s/' % (jobid, channel)
