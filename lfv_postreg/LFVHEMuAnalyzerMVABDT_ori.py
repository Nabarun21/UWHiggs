...
#                self.book(f, "jetN_20", "Number of jets, p_{T}>20", 10, -0.5, 9.5) 
    ...
    def fill_histos(self, row,MVAvalue,sign,f=None,fillCommonOnly=False,region=None,btagweight=1,sys='',qcdshape=None,pileup='nominal'):
        ...
        if self.is_WJet or self.is_DYJet or self.is_DYlowmass or self.is_ZTauTau:
            ...
            weight=self.GluGluHTo_weight*self.event_weight(row,region,pileup) 
            ...
            weight=self.GluGlu_LFV_HToMuTau_weight*self.event_weight(row,region,pileup) 
        elif self.isVBFHTo:
            weight=self.VBFHTo_weight*self.event_weight(row,region,pileup) 
        elif self.isVBF_LFV:
            weight=self.VBF_LFV_HToMuTau_weight*self.event_weight(row,region,pileup) 
...
#            if btagweight<0:btagweight=0
            ...
            for sys in self.sysdir:
                ...
                self.my_elec_uncorr=ROOT.TLorentzVector()
                self.my_elec_uncorr.SetPtEtaPhiM(row.ePt_ElectronUncorr,row.eEta,row.ePhi,row.eMass)
                
                self.my_MET=ROOT.TLorentzVector()
                self.my_MET.SetPtEtaPhiM(row.type1_pfMetEt,0,row.type1_pfMetPhi,0)

                self.my_METpx=self.my_MET.Px()+self.my_elec_uncorr.Px()-self.my_elec.Px()
                self.my_METpy=self.my_MET.Py()+self.my_elec_uncorr.Py()-self.my_elec.Py()
...
#                print sys,"     ",MVAval
                ...
                if jn!=3:
                    self.fill_histos(row,MVAval,sign,None,True,region,btagweight,sys,qcdshaperegion,self.pileup)

                folder = sign+'/'+processtype+'/'+str(int(jn))+'/selected/'+sys
                self.fill_histos(row,MVAval,sign,folder,False,region,btagweight,sys,qcdshaperegion,self.pileup)
           ...
           #take now divide by e-mu isolation regions, looseloose,loosetight,tightloose,tighttight
            ...
            for jetsys in self.jetsysdir:
                ...
                if jn!=3:
                    self.fill_histos(row,MVAval,sign,None,True,region,btagweight,jetsys,qcdshaperegion,self.pileup)
                        
            
                folder = sign+'/'+processtype+'/'+str(int(jn))+'/selected/'+jetsys
                self.fill_histos(row,MVAval,sign,folder,False,region,btagweight,jetsys,qcdshaperegion,self.pileup)
        cut_flow_trk.flush()        
