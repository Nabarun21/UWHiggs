...
python getQCD_new.py $analyzer $luminosity $jobid $analtype $sys 
python getQCDforcombine_new.py $analyzer $luminosity $jobid $analtype $sys   #get the same without sysuncertainty in histos, combine script already adds them
...
python prepHistos_new.py $analyzer $luminosity $jobid $analtype
