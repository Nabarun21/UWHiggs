...
class LFVHEMuAnalyzerMVABDTcommonvar(MegaBase):
    tree = 'em/final/Ntuple'
    def __init__(self, tree, outfile, **kwargs):

        self.channel='EMu'
        super(LFVHEMuAnalyzerMVABDTcommonvar, self).__init__(tree, outfile, **kwargs)
        target = os.path.basename(os.environ['megatarget'])
        self.target=target
        self.var =['mTmuMet_','mTeMet_','deltaPhimue_','eDphiPFMet_','mDphiPFMet_','mColl_','mPt_','ePt_'] 
        self.xml_name = os.path.join(os.getcwd(),"BDT_B2/weights/TMVAClassification_BDT.weights.xml")  #weights from BDT
...
#            self.sysdir=['nosys','mesup']
            self.sysdir=['nosys','uup','udown','chargeduesdown','chargeduesup','ecaluesdown','ecaluesup','hcaluesdown','hcaluesup','hfuesdown','hfuesup','mesup','mesdown','eesup','eesdown','eresup','eresdown','eEtdependent','enomsmear','puup','pudown']
...
#
                ...
                self.book(f, "pZeta",  "pZeta",  40,-200, 200) 
...
#                self.book(f, "jetN_20", "Number of jets, p_{T}>20", 10, -0.5, 9.5) 
        ...
        for s in sign:
            ...
            self.book(s, "BDT_value",  "BDT_value",  60, -0.6, 0.6)
            self.book(s, "Met",  "Met",  30, 0, 300) 
            self.book(s, "pZeta",  "pZeta",  40,-200, 200) 
            self.book(s, "mPFMET_Mt", "mu-PFMET M_{T}" , 200, 0, 200)
            self.book(s, "ePFMET_Mt", "e-PFMET M_{T}" , 200, 0, 200)           
            self.book(s, "mPFMET_DeltaPhi", "mu-PFMET DeltaPhi" , 50, 0, 3.2)
            self.book(s, "ePFMET_DeltaPhi", "e-PFMET DeltaPhi" , 50, 0, 3.2)
            self.book(s,"mPt", "mu p_{T}", 150, 0, 150)
            self.book(s,"mPhi", "mu phi", 100, -3.2, 3.2)
            self.book(s,"mEta", "mu eta",  50, -2.5, 2.5)
            
            self.book(s,"ePt", "e p_{T}", 150, 0, 150)
            ...
            for sys in self.sysdir:
                self.book(s+'/'+sys, "h_vismass",  "h_vismass",  300, 0, 300)
                self.book(s+'/'+sys, "h_collmass_pfmet", "h_collmass_pfmet", 300, 0, 300)
                self.book(s+'/'+sys, "BDT_value",  "BDT_value",  60, -0.6, 0.6)
            for jetsys in self.jetsysdir:
                self.book(s+'/'+jetsys, "h_vismass",  "h_vismass",  300, 0, 300)
                self.book(s+'/'+jetsys, "h_collmass_pfmet", "h_collmass_pfmet", 300, 0, 300)
                self.book(s+'/'+jetsys, "BDT_value",  "BDT_value",  60, -0.6, 0.6)
        ...
        if fillCommonOnly==True:
            if region=='signal':
                if sys=='presel':
                    ...
                    self.histograms[sign+'/BDT_value'].Fill(MVAvalue,weight)
                    ...
                    self.histograms[sign+'/pZeta'].Fill(row.e_m_PZetaLess0p85PZetaVis, weight)
...
#            if btagweight<0:btagweight=0
            ...
            for sys in self.sysdir:
                ...
                if sys =='nosys':
                    ...
                    self.shifted_type1_pfMetPhi=row.type1_pfMet_shiftedPhi_JetCheckUESUp
                    self.shifted_type1_pfMetEt=row.type1_pfMet_shiftedPt_JetCheckUESUp
                    ...
                    self.shifted_type1_pfMetPhi=row.type1_pfMet_shiftedPhi_JetCheckUESDown
                    self.shifted_type1_pfMetEt=row.type1_pfMet_shiftedPt_JetCheckUESDown
                    self.my_MET.SetPtEtaPhiM(self.shifted_type1_pfMetEt,0,self.shifted_type1_pfMetPhi,0)
                    ...
                    if abs(self.my_elec.Eta()<1.479):
                        ...
                        my_rand_num=self.generator.Gaus(1,0.01246)
                        
                        self.my_elec.SetPxPyPzE(self.my_elec.Px()*my_rand_num,self.my_elec.Py()*my_rand_num,self.my_elec.Pz()*my_rand_num,self.my_elec.E()*my_rand_num)

                        #self.my_elec.SetPtEtaPhiM(row.ePt*self.generator.Gaus(1,0.01246),row.eEta,row.ePhi,row.eMass)
                        ...
                        self.my_METpx=self.my_MET.Px()+self.my_elec.Px()
                        self.my_METpy=self.my_MET.Py()+self.my_elec.Py()
#                        print "MET  ",self.my_MET.Px(),"   ",self.my_MET.Py(),"   ",self.my_MET.Phi()
#                        print "elec   ",self.my_elec.Px(),"   ",self.my_elec.Py(),"   ",self.my_elec.Phi()
#                        print " "
                        my_rand_num=self.generator.Gaus(1,0.02546)
                        self.my_elec.SetPxPyPzE(self.my_elec.Px()*my_rand_num,self.my_elec.Py()*my_rand_num,self.my_elec.Pz()*my_rand_num,self.my_elec.E()*my_rand_num)
                        self.my_METpx-=self.my_elec.Px()
                        self.my_METpy-=self.my_elec.Py()
...
#                        print "elec   ",self.my_elec.Px(),"   ",self.my_elec.Py(),"   ",self.my_elec.Phi()
#                        print " "
                        my_rand_num=self.generator.Gaus(1,0.01254)
                        self.my_elec.SetPxPyPzE(self.my_elec.Px()*my_rand_num,self.my_elec.Py()*my_rand_num,self.my_elec.Pz()*my_rand_num,self.my_elec.E()*my_rand_num)
...
#                        print "elec   ",self.my_elec.Px(),"   ",self.my_elec.Py(),"   ",self.my_elec.Phi()
#                        print " "
                        my_rand_num=self.generator.Gaus(1,0.02554)
                        self.my_elec.SetPxPyPzE(self.my_elec.Px()*my_rand_num,self.my_elec.Py()*my_rand_num,self.my_elec.Pz()*my_rand_num,self.my_elec.E()*my_rand_num)
...
#                        print "elec   ",self.my_elec.Px(),"   ",self.my_elec.Py(),"   ",self.my_elec.Phi()
#                        print " "
                        my_rand_num=self.generator.Gaus(1,0.01250)
                        self.my_elec.SetPxPyPzE(self.my_elec.Px()*my_rand_num,self.my_elec.Py()*my_rand_num,self.my_elec.Pz()*my_rand_num,self.my_elec.E()*my_rand_num)
...
#                        print "elec   ",self.my_elec.Px(),"   ",self.my_elec.Py(),"   ",self.my_elec.Phi()
#                        print " "
                        my_rand_num=self.generator.Gaus(1,0.02550)
                        self.my_elec.SetPxPyPzE(self.my_elec.Px()*my_rand_num,self.my_elec.Py()*my_rand_num,self.my_elec.Pz()*my_rand_num,self.my_elec.E()*my_rand_num)
                        self.my_METpx-=self.my_elec.Px()
                        self.my_METpy-=self.my_elec.Py()
                        self.my_MET.SetPxPyPzE(self.my_METpx,self.my_METpy,0,sqrt(self.my_METpx*self.my_METpx+self.my_METpy*self.my_METpy))
...
#                        print " "
                        ...
                        my_rand_num=self.generator.Gaus(1,0.01250*(float(6.6)/float(sqrt(row.ePt))))
                        ...
                        self.my_METpx=self.my_MET.Px()+self.my_elec.Px()
                        self.my_METpy=self.my_MET.Py()+self.my_elec.Py()
                        my_rand_num=self.generator.Gaus(1,0.02550*(float(6.6)/float(sqrt(row.ePt))))
...
#                print sys,"     ",MVAval
                ...
                if sys=='nosys':    
                    if jn<3:
                        self.fill_histos(row,MVAval,sign,None,True,region,btagweight,'presel')
                ...
                if jn<3:
                    self.fill_histos(row,MVAval,sign,None,True,region,btagweight,sys,qcdshaperegion)
           ...
           #take now divide by e-mu isolation regions, looseloose,loosetight,tightloose,tighttight
            ...
            for jetsys in self.jetsysdir:
                ...
                if jn<3:
                    self.fill_histos(row,MVAval,sign,None,True,region,btagweight,jetsys,qcdshaperegion)
