...
class LFVHEMuAnalyzerMVABDTR04(MegaBase):
    tree = 'em/final/Ntuple'
    def __init__(self, tree, outfile, **kwargs):

        self.channel='EMu'
        super(LFVHEMuAnalyzerMVABDTR04, self).__init__(tree, outfile, **kwargs)
        ...
        self.WZTo1L1Nu2Q_weight=2.54725706507e-08  #2.54725706507e-08
...
#        print eisocorr
        ...
        return pu*muidcorr*muisocorr*mutrcorr*topptreweight*eidcorr*ereconcorr
   ...
   #                 self.shifted_jetVeto30=row.jetVeto30_JetEnDown
    ...
    #                self.shifted_vbfDeta=row.vbfDeta_JetEnDown
           ...
           #take care of ecal gap
                ...
                if (getattr(row,'mRelPFIsoDBDefaultR04')>0.15) and (not getattr(row,'mRelPFIsoDBDefault')>0.15):
                    print 'isolation values different: 04 passes but regular doesnot'
                    print 'regular: ',getattr(row,'mRelPFIsoDBDefault')
                    print '04: ',getattr(row,'mRelPFIsoDBDefaultR04')
                if (not getattr(row,'mRelPFIsoDBDefaultR04')>0.15) and (getattr(row,'mRelPFIsoDBDefaultR04')>0.15):
                    print 'isolation values different; reular passes but 04 doesnot'
                    print 'regular: ',getattr(row,'mRelPFIsoDBDefault')
                    print '04: ',getattr(row,'mRelPFIsoDBDefaultR04')
